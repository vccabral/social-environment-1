...
def create_air_quality_database(apps, schema_editor):
    ...
    import_all = True
    years_range = range(1987, 2015) if import_all else range(2010, 2011)
    redo_all = True
    download_again = True
    import_toxic = True
    import_air = True
                ...
                for line in f: 
                    ...
                    if is_first_line:
                        ...
                        p = ToxicDataPoint(
                            ...
                            LATITUDE = encode_for_database(columns[42]) if encode_for_database(columns[42]) != '' else None,
                            LONGITUDE = encode_for_database(columns[43]) if encode_for_database(columns[43]) !='' else None,
                ...
                for columns in lines:
                    ...
                    if is_first_line:
                        ...
                        p = AirQualityDataPoint(
                            ...
                            Latitude = encode_for_database(columns[5]) if encode_for_database(columns[5]) != '' else None,
                            Longitude = encode_for_database(columns[6]) if encode_for_database(columns[6]) !='' else None,
